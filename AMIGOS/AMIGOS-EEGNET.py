...
from EEGModels import DeepConvNet, ShallowConvNet, EEGNet
...
for i in train_subject:
    ...
    for j in range(1, num_trial + 1):
        ...
        while True:
            ...
            '''
            # arousal
            if self_assment[0] >= 4.5:
                self_assment[0] = 1
            else:
                self_assment[0] = 0
            '''
            # valence
            if self_assment[1] >= 4.5:
                self_assment[1] = 1
            else:
                self_assment[1] = 0
            '''
...
########################################################################################################################
model = EEGNet(nb_classes=num_classes, Chans=channel, Samples=sample,
...
Model_path = Model_save_path + 'EEGNet(AMIGOS).hdf5'
